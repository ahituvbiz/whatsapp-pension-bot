...
def build_investment_text(data, analysis, user_profile):
    ...
    if age <= 52:
        if neq:
            ...
            ip   = round(ifv / mult)
            lines.append(
                f"\n💡 בהתחשב בטווח השנים שנותר לך עד לפרישה אני ממליץ "
                f"{g(gender,'שתבחר','שתבחרי')} במסלול או במסלולים מנייתיים בלבד."
            )
            lines.append(
                "ב-5 השנים האחרונות המסלולים המנייתיים הניבו תשואה עודפת של כ-1.25% "
                "ביחס למסלולי 'בני 50 ומטה' (ופער גדול יותר מול יתר המסלולים)."
            )
            lines.append(f"הקצבה מהכספים שצברת עד סוף תקופת הדוח היא ₪{format_number(p67)}.")
            lines.append(
                f"אם {g(gender,'תשפר','תשפרי')} את התשואה ב-1.25% הקצבה על אותם כספים תגדל ל-₪{format_number(ip)}."
            )
            lines.append(
                "כמובן שהפער יהיה גדול יותר כי התשואה העודפת תהיה גם על הכספים שיופקדו לקרן בעתיד."
            )
            if eq and age_track:
                non_eq_names = ", ".join(neq)
                deposit_source = analysis.get("deposit_source", "שכיר")
                if deposit_source == "עצמאי":
                    lines.append(
                        f"\nℹ️ הכסף שלך מפוצל בין מסלול מנייתי לבין מסלול שאיננו מנייתי ({non_eq_names})."
                    )
                    lines.append("סביר להניח שבמסלול הלא מנייתי נמצאים כספי פיצויים.")
                    lines.append(
                        f"{g(gender,'בדוק','בדקי')} עם הקרן מה צריך לעשות בכדי להעביר אותם "
                        f"למסלול ההשקעה {g(gender,'שבחרת','שבחרת')}."
                    )
                else:
                    lines.append(
                        f"\nℹ️ הכסף שלך מפוצל בין מסלול מנייתי לבין מסלול שאיננו מנייתי ({non_eq_names})."
                    ...
                    lines.append(
                        "סביר להניח שבמסלול הלא מנייתי נמצאים כספי הפיצויים שלך "
                        "המהווים כ-30%-40% מהפנסיה שלך."
                    )
                    lines.append(
        ...
        lines.append(
            "\n💡 בהתחשב בשנים שנותרו לך לפרישה יש לבחון איך להתאים את שיעור החשיפה למניות "
            "כך שהוא יפחת באופן הדרגתי לקראת היציאה לפנסיה."
        )
    if sp500:
        lines.append(
            f"\n⚠️ {g(gender,'אתה מושקע','את מושקעת')} במדד S&P 500. "
            "מדד זה סובל היום מריכוזיות גבוהה."
        )
        lines.append(
            "המשקל של 9 החברות הגדולות הכלולות בו הוא כ-40% מה שמגדיל את הסיכון הכרוך בהשקעה בו."
        )
        lines.append(
            "בנוסף התימחור של המניות הכלולות בו משקף מידה רבה של אופטימיות "
            "שאם היא תתברר כמוגזמת התשואה שהמדד יניב תהיה נמוכה."
        )
        lines.append("מעבר לכך אין לך חשיפה למניות של חברות מוצלחות מיתר העולם.")
    if madedei and fk in MADEDEI_WARNING_FUNDS:
        lines.append(
            "\n⚠️ בפנסיה אנחנו צריכים לנהוג בזהירות. "
            "מסלול מדדי מניות הוא מסלול עם רמת ריכוז גבוהה מאוד וממילא סיכון גבוה."
        )
        lines.append(
            "המניות הכלולות בו אמנם הניבו תשואה חריגה בעבר אבל זה כרוך בסיכון "
            "שלגמרי לא בטוח שמתאים לכספי פנסיה."
        )
    if halacha and age <= 52 and fk != "אינפיניטי":
        lines.append(
            f"\n⚠️ במסלול ההלכה בקרן בה {g(gender,'אתה נמצא','את נמצאת')} "
            "החשיפה למניות איננה מקסימלית."
        )
        lines.append("הצפי הוא שהתשואה שלך תהיה נמוכה יותר בגלל זה.")
        lines.append(
            f"{g(gender,'שקול','שיקלי')} לעבור למסלול מותאם להלכה שיש בו חשיפה מנייתית מקסימלית "
            "בכדי לשפר את הפנסיה שלך."
        )
